...
class ImageDuplicateFinder {
     ...
     */
    async findSimilarImages(imagePath, maxResults = 10, similarityThreshold = 0.8) {
        ...
        try {
            ...
            // 提取查询图片的特征
            const queryFeatures = await this.imageProcessor.extractFeatures(imagePath);
            const queryMetadata = await this.imageProcessor.getImageMetadata(imagePath);
    ...
    async checkOllamaService() {
        try {
            // 尝试创建一个简单的嵌入来测试服务
            const testEmbedding = await this.imageProcessor.embeddings.embedQuery('test');
